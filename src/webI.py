...
def detectWAF(url, port, lang):
    ...
    ack_flag_scan_resp = sr1(
        IP(dst=dst_ip)/TCP(dport=port, flags="A"), timeout=10, verbose=0)
...
def getParams(htmlText):
    ...
    if len(fieldPos) == 0:
        delimiter = "\'"
        fieldPos = [m.start()
                    for m in re.finditer(textBox+delimiter, htmlText)]
...
def testHttps(url, lang):
    ...
    try:
        ...
        response = _('Resposta: ') + str(r) + buildResponse(str(r) ==
                                                            '<Response [200]>', _('\nHttps OK'), _('\nHttps nao utilizado'))
        pass
        ...
        if '[' in err:
            err = err.split("[", 1)[1]
            err = err.split("]", 1)[0]
        response = _('Resposta: ') + err + \
            buildResponse(False, '', _('\nHttps nao utilizado'))
        pass
    return response
...
def testClickJack(url, lang):
    ...
    if validation == True:
        validation = (r.headers['X-Frame-Options'] ==
                      'DENY') | (r.headers['X-Frame-Options'] == 'SAMEORIGIN')
    response = _('Resposta: ') + str(r) + buildResponse(validation,
...
def testSQLi(url, paths, lang):
    ...
    print url
    pageList = []
    msg = ["Nao foram encontrados indicios de um DBMS no codigo",
           "O teste gerou um acesso indevido ao banco de dados"]
    msgIndex = 0
    ...
    for urlTest in paths:
        ...
        if response.status_code == 200:
            ...
            print _('Testando ' + bcolors.UNDERLINE + urlTest +
                    bcolors.ENDC + ' - Campos: ' + ' '.join(str(p) for p in params))
...
def testXSS(url, lang):
    ...
    # Input method
    while selection == 1:
        path = '/'+ raw_input(_('Insira um caminho: '))
        if not path in paths:
            paths.append(path)
            pass
        selection = raw_input(
            _('Deseja inserir mais caminhos? \n1 - Sim\n2 - Nao\n'))
    ...
    # Making the result
    response = _('Resposta: ') + msg[msgIndex] + buildResponse(
...
def webIDirect(lang):
    ...
    exitOpt = raw_input(
        _('Deseja incluir algum caminho para teste?:\n1 - Sim\n2 - Nao\n'))
...
def xss(url):
    ...
    for testString in inputTests:
        ...
        try:
            ...
            results = session.get(url + testString, headers=hdr)
