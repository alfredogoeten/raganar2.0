...
def transCryptDirect(lang):
    ...
    src_port = RandShort()
    response = sr1(IP(dst=url) / TCP(sport=src_port,
                                     dport=int(port), flags="S"), timeout=3, verbose=0)
    ...
    # Stealth response
    send_rst = sr(IP(dst=url) / TCP(sport=src_port,
                                    dport=int(port), flags="R"), timeout=3, verbose=0)
    clear()
    ...
    print bcolors.HEADER + '[2] ' + bcolors.ENDC + \
          (_('Protocolo padrao do servidor'))
    ...
    print bcolors.HEADER + '[3] ' + bcolors.ENDC + \
          (_('Uso the Perfect Forward Secrecy'))
    ...
    # [5] Test Weak Ciphers
    printLine()
    print bcolors.HEADER + '[4] ' + bcolors.ENDC + \
          (_('Testando Cifragem Fraca  (minimo 128bits) '))
...
def testPFS(host, port, protocol):
    ...
    return buildResponse(foundOne, _('PFS habilitado com as cifras: ') + pfsCipherOk,
                         _('PFS nao detectado com as cifras testadas'))
...
def testWeakCipher(host, port, protocolList):
    ...
    for protoData in protoDataList:
        print protoData[0] + '\t\t' + str(protoData[1]) + ' bits' + ('(OK)' if (
                protoData[1] >= 128) else _('(FRACA)')) + '\the\t' + str(protoData[2])
...
def transCryptMenu(lang):
    ...
    while True:
        ...
        opt = int(
            raw_input(_("0 - Voltar para o Menu Principal\n1 - Verificar SSL/TLS\n")))
        if opt == 0:
            return
        if opt in menuOpts:
...
def verifyProtocol(host, port, protoVersion):
    ...
    packet = 'GET / HTTP/1.0\r\n\r\n' if (port ==
                                          80 or port == 443) else '<packet>SOME_DATA</packet>'
