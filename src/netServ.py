    ...
    try:
        ...
        for port in portRange:
            ...
            stealth_scan_resp = sr1(
                IP(dst=server) / TCP(sport=src_port, dport=port, flags="S"), timeout=3, verbose=0)
            ...
            if (str(type(stealth_scan_resp)) == "<type 'NoneType'>"):
                if ((not ignoreFilter) and showMsg):
                    print "{} \t\t".format(port) + (
                        (COMMON_PORTS[port] + ("\t" if len(COMMON_PORTS[port]) < 8 else "")) if (
                            port in COMMON_PORTS) else "-\t") + _("\t\tFiltrada")

            # Analyse the TCP layer (if there is one)
            elif (stealth_scan_resp.haslayer(TCP)):
                if (stealth_scan_resp.getlayer(TCP).flags == 0x12):
                    # Stealth response
                    send_rst = sr(IP(dst=server) / TCP(sport=src_port,
                                  dport=port, flags="R"), timeout=3, verbose=0)
                    if showMsg:
                        print "{} \t\t".format(port) + (
                            (COMMON_PORTS[port] + ("\t" if len(COMMON_PORTS[port]) < 8 else "")) if (
                                port in COMMON_PORTS) else "-\t") + _("\t\tAberta")
        ...
        if scanCommon:
            for port in {k for k, v in COMMON_PORTS.iteritems() if k > 1025}:
                if port > 1025:
                    ...
                    stealth_scan_resp = sr1(IP(dst=server) / TCP(sport=src_port, dport=port, flags="S"), timeout=3,
                                            verbose=0)
                    ...
                    if (str(type(stealth_scan_resp)) == "<type 'NoneType'>"):
                        ...
                        if (stealth_scan_resp.getlayer(TCP).flags == 0x12):
                            # Stealth response
                            send_rst = sr(IP(dst=server) / TCP(sport=src_port, dport=port, flags="R"), timeout=3,
                                          verbose=0)
                            if showMsg:
                                print "{} \t\t".format(port) + (
                                    (COMMON_PORTS[port] + ("\t" if len(COMMON_PORTS[port]) < 8 else "")) if (
...
def netServMenu():
    ...
    while True:
        ...
        opt = int(raw_input(
            _("0 - Voltar para o Menu Principal\n1 - Verificar Servicos de Rede\n")))
        if opt == 0:
            return
        if opt in menuOpts:
...
def testDDoS(url, openPorts):
    ...
    for port in openPorts:
        ...
        print str(httpAtkResponse[0]) + _(' tentativas, ') + str(httpAtkResponse[1]) + (
...
def sendFuzz(url, port, startString, endString, secondsOfTest):
    ...
    while (time.time() < stopTime):
        ...
        try:
            ...
            failCount = 0
            randomString = ''.join(random.choice(string.uppercase)
                                   for i in range(50))
        ...
        # If we fail to connect to the server, we assume its crashed and print the statement below
            ...
            failCount += 1
            if (failCount == 3):
                print _('Porta ') + str(port) + _(': 3 erros consecutivos com string de tamanho ') + str(
                    len(testString + endString) - 50) + _('\nExcecao: ') + str(e)
                return True
...
def testOpenPorts(url, openPorts):
    ...
    for port in openPorts:
        ...
        print "[+] Port {} [".format(port) + (
            (COMMON_PORTS[port] + ']:' + ("\t" if len(COMMON_PORTS[port] + ']') < 7 else '\t')) if (
                port in COMMON_PORTS) else "-]\t") + "\t\tOpen"
        ...
        if (port == 23) | (port == 69):  # Verify unencrypted and unauthenticated ports
            problemDetect = True
            warningMsg += '- \t' + \
                COMMON_PORTS[port] + _(' - Porta [{}] Exposta\n').format(port)
