...
class No(object):
    ...
    def inicia_server(self):
        ...
        s_escuta = bind_socket(
            s_escuta, IP_LOCAL, PORTA_BASE_ESCUTA_NOS + self.no_id)
        ...
        try:
            ...
            while True:
                ...
                start_new_thread(self.parse_mensagem_recebida,
                                 (dados, tupla_endereco_porta, s_escuta))
    ...
    def envia_mensagem_no(self, tipo_msg, tupla_endereco, id_destino, mensagem_complemento=""):
        ...
        mensagem = TIPO + SEPARADOR_ATRIBUTO_VALOR + tipo_msg + SEPARADOR_MSG +\
            ID_NO + SEPARADOR_ATRIBUTO_VALOR + \
            str(self.no_id) + mensagem_complemento

        tupla_endereco_porta = (
            tupla_endereco[0], PORTA_BASE_ESCUTA_NOS + id_destino)
        nao_enviado = True
        contador = 0
        while nao_enviado:
            try:
    ...
    def troca_proximo_no_dht(self, tupla_endereco_porta, s_escuta, mensagem_atributo_valor, id_no_valor):
        ...
        novo_id_prox, novo_endereco_proximo = self.parse_mensagem_novo_proximo(
            mensagem_atributo_valor)
        if novo_id_prox == self.no_id:
            ...
            self.envia_mensagem_no(
                TIPO_SOU_SEU_ANT, self.tupla_no_prox_endereco, self.no_prox_id)
    ...
    def parse_mensagem_recebida(self, mensagem, tupla_endereco_porta, s_escuta):
        ...
        tipo, tipo_valor = mensagem_atributo_valor[0].split(
            SEPARADOR_ATRIBUTO_VALOR)
        idNo, id_no_valor = mensagem_atributo_valor[1].split(
            SEPARADOR_ATRIBUTO_VALOR)

        if tipo_valor == TIPO_CONTATO_INICIAL_ROOT:
            self.contato_inicial_com_root(
                s_escuta, tupla_endereco_porta, id_no_valor)
            self.atualiza_no_sucessor_proximo()
        elif tipo_valor == TIPO_ACHA_PROX:
            self.responde_acha_proximo(
                s_escuta, tupla_endereco_porta, id_no_valor)
            ...
            self.troca_proximo_no_dht(
                tupla_endereco_porta, s_escuta, mensagem_atributo_valor, id_no_valor)
            self.atualiza_no_sucessor_proximo()
        elif tipo_valor == TIPO_CONT_BUSCA:
            end, end_valor = mensagem_atributo_valor[2].split(
                SEPARADOR_ATRIBUTO_VALOR)
            porta, porta_valor = mensagem_atributo_valor[3].split(
                SEPARADOR_ATRIBUTO_VALOR)
            id_a, id_arquivo = mensagem_atributo_valor[4].split(
                SEPARADOR_ATRIBUTO_VALOR)
            id_o, id_origem = mensagem_atributo_valor[5].split(
                SEPARADOR_ATRIBUTO_VALOR)
            print "Pedido de busca recebido -> Arquivo a buscar: " + \
                id_arquivo+" Responder para No: "+id_origem
            self.continua_busca(
                int(id_arquivo), [end_valor, int(porta_valor)], int(id_origem))
        elif tipo_valor == TIPO_RESPOSTA_BUSCA:
            id_e, id_encontrado = mensagem_atributo_valor[2].split(
                SEPARADOR_ATRIBUTO_VALOR)
            conteudo, conteudo_arquivo = mensagem_atributo_valor[3].split(
                SEPARADOR_ATRIBUTO_VALOR)
            print "Arquivo encontrado no noh de id: "+id_encontrado + \
                " com o seguinte conteudo: "+conteudo_arquivo
    ...
    def responde_acha_proximo(self, s, tupla_endereco_porta, id_no_contatante):
        reply = ID_NO_PROX + SEPARADOR_ATRIBUTO_VALOR + str(self.no_prox_id) + SEPARADOR_MSG +\
            ...
            PORTA_PROX + SEPARADOR_ATRIBUTO_VALOR + \
            str(self.tupla_no_prox_endereco[1]) + SEPARADOR_MSG
    ...
    def parse_mensagem_novo_proximo(self, mensagem_atributo_valor):
        tipo, novo_id_prox = mensagem_atributo_valor[2].split(
            SEPARADOR_ATRIBUTO_VALOR)
        ip, ip_no_valor = mensagem_atributo_valor[3].split(
            SEPARADOR_ATRIBUTO_VALOR)
        porta, porta_no_valor = mensagem_atributo_valor[4].split(
            SEPARADOR_ATRIBUTO_VALOR)
    ...
    def atualiza_no_sucessor_proximo(self):
        if not self.no_prox_id == -1:
            ...
            id_no_sucessor, tupla_endereco_sucessor = self.acha_proximo(
                self.no_prox_id, self.tupla_no_prox_endereco)
    ...
    def entra_primeiro_no_dht(self, id_no_contatante, tupla_endereco_no_contatante):
        self.atualiza_no_proximo(
            id_no_contatante, tupla_endereco_no_contatante)
        self.atualiza_no_anterior(
            id_no_contatante, tupla_endereco_no_contatante)
    ...
    def entra_dht(self):  # inserindo um No na rede
        # IP do root mais a porta que ele escuta, que eh 5000 + numero de id
        parse_reply = self.envia_mensagem_no(
            TIPO_CONTATO_INICIAL_ROOT, self.tupla_no_root_endereco, self.no_root_id)

        status_dht_atributo, status_dht_valor = parse_reply[0].split(
            SEPARADOR_ATRIBUTO_VALOR)
        ...
        while not acha_lugar:
            ...
            if id_no_atual < id_no_prox:
                if (id_no_atual < self.no_id and self.no_id < id_no_prox):
                    ...
                    self.atualiza_no_anterior(
                        id_no_atual, tupla_endereco_atual)
                    ...
                    self.envia_mensagem_no(
                        TIPO_SOU_SEU_PROX, tupla_endereco_atual, id_no_atual)
                    self.envia_mensagem_no(
                        TIPO_SOU_SEU_ANT, tupla_endereco_prox, id_no_prox)
                else:
                    id_no_atual, tupla_endereco_atual = id_no_prox, tupla_endereco_prox
                    id_no_prox, tupla_endereco_prox = self.acha_proximo(
                        id_no_prox, tupla_endereco_prox)
            else:
    ...
    def acha_proximo(self, id_no_prox, tupla_endereco_prox):
        parse_reply = self.envia_mensagem_no(
            TIPO_ACHA_PROX, tupla_endereco_prox, id_no_prox)
    ...
    def parse_resposta_rendezvous(self, resposta):
        ...
        if str_root == EH_ROOT:
            ...
            atr_id_root, str_id_root = lista_mensagem[2].split(
                SEPARADOR_ATRIBUTO_VALOR)
            self.no_root_id = int(str_id_root)

            atr_endereco, str_endereco = lista_mensagem[3].split(
                SEPARADOR_ATRIBUTO_VALOR)
            atr_porta, str_porta = lista_mensagem[4].split(
                SEPARADOR_ATRIBUTO_VALOR)
            self.tupla_no_root_endereco = (str_endereco, int(str_porta))

    def envia_hello(self, s_envia):
        s_envia.sendto(MENSAGEM_INICIAL,
                       (IP_ACESSAR_RENDEZVOUS, PORTA_RENDEZVOUS))
        print_msg_enviada(MENSAGEM_INICIAL,
                          (IP_ACESSAR_RENDEZVOUS, PORTA_RENDEZVOUS))
        ...
        if reply is not None:
            ...
            s_envia.sendto(
                ACKNOWLEDGE, (IP_ACESSAR_RENDEZVOUS, PORTA_RENDEZVOUS))
            print_msg_enviada(
                ACKNOWLEDGE, (IP_ACESSAR_RENDEZVOUS, PORTA_RENDEZVOUS))
        else:
            return True
    ...
    def sair_dht(self):
        mensagem_complemento = SEPARADOR_MSG + "novo_id_prox" + SEPARADOR_ATRIBUTO_VALOR + str(self.no_prox_id) + SEPARADOR_MSG +\
            ...
            "novo_porta_prox" + SEPARADOR_ATRIBUTO_VALOR + \
            str(self.tupla_no_prox_endereco[1])
    ...
    def calcula_id_no_arquivo(self, id_arquivo):
        posicao = int(
            floor((id_arquivo - 1)/(MAX_ARQUIVOS/(MAX_SEQUENCIAL+1))))
        print posicao
    ...
    def inicia_busca(self):
        ...
        print "Numero de arquivos armazenados localmente: " + \
            str(len(self.arquivos_armazenados))
        tupla_endereco_no_origem = [
            IP_LOCAL, PORTA_BASE_ESCUTA_NOS + self.no_id]
        print "Meu Id: " + str(self.no_id) + " - Id Responsavel: " + \
            str(id_no_responsavel) + " - Proximo Id: " + str(self.no_prox_id)
        if self.no_id == id_no_responsavel:
            print "Este noh (ID" + str(self.no_id) + \
                ") eh o responsavel pelo arquivo"
            ...
            if self.no_id > self.no_prox_id:
                if self.no_prox_id > id_no_responsavel:
                    ...
                    for i in range(0, len(self.arquivos_armazenados)-1):
                        if self.arquivos_armazenados[i][0] == id_arquivo:
                            print("Sucesso, arquivo encontrado no id " +
                                  str(self.no_id))
                            return
                    print "Porem o arquivo nao se encontra armazenado na tabela"
                    return
                else:
                    print "Enviar pedido de busca para o no " + \
                        str(self.no_prox_id)
                    self.envia_continua_busca(TIPO_CONT_BUSCA, self.tupla_no_prox_endereco,
                                              self.no_prox_id, id_arquivo, tupla_endereco_no_origem, self.no_id)
            else:
                print "Enviar pedido de busca para o no " + \
                    str(self.no_prox_id)
                self.envia_continua_busca(TIPO_CONT_BUSCA, self.tupla_no_prox_endereco, int(
                    self.no_prox_id), id_arquivo, tupla_endereco_no_origem, self.no_id)
        else:
    ...
    def continua_busca(self, id_arquivo, tupla_endereco_no_origem, id_origem):
        ...
        print "Numero de arquivos armazenados localmente: " + \
            str(len(self.arquivos_armazenados))
        if self.no_id == id_no_responsavel:
            self.envia_resposta_busca(
                TIPO_RESPOSTA_BUSCA, tupla_endereco_no_origem, id_origem, self.no_id, "Arquivo "+str(id_arquivo))
            return
            # for i in range(0,len(self.arquivos_armazenados)-1):
            #    if self.arquivos_armazenados[i][0] == id_arquivo:
            ...
            if self.no_id > self.no_prox_id:
                if self.no_prox_id > id_no_responsavel:
                    ...
                    print "Enviando resposta de busca para o no " + \
                        str(id_origem)
                    self.envia_resposta_busca(
                        TIPO_RESPOSTA_BUSCA, tupla_endereco_no_origem, id_origem, self.no_id, "Arquivo "+str(id_arquivo))
                    return
                    ...
                    self.envia_continua_busca(TIPO_CONT_BUSCA, self.tupla_no_prox_endereco,
                                              self.no_prox_id, id_arquivo, tupla_endereco_no_origem, id_origem)
            else:
                self.envia_continua_busca(TIPO_CONT_BUSCA, self.tupla_no_prox_endereco,
                                          self.no_prox_id, id_arquivo, tupla_endereco_no_origem, id_origem)
        else:
